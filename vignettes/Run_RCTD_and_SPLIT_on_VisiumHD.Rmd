...
SPLIT no longer requires RCTD output as of version `0.2.0`. You can now supply any cell-type deconvolution results via a `deconvolution_weights` matrix `(cells × cell_types)` and a `reference` matrix `(genes × cell_types)`. Optionally, provide `primary_cell_type`; if omitted, it is inferred per cell as `argmax(deconvolution_weights)`. By default, all cells are purified toward their `primary_cell_type`, treating the remaining non-zero weights as proportional contamination from the corresponding cell types. To control which cells to purify, pass their identifiers to `cells_to_purify` argument. 

```{r SPLIT, message=TRUE}
converted_rctd <- convert_rctd_result_to_purify_input(RCTD)

reference <- t(converted_rctd$reference)
primary_cell_type <- converted_rctd$primary_cell_type
deconvolution_weights <- converted_rctd$deconvolution_weights
...
p3 <- UMAPPlot(vhd_purified, group.by = c("purification_status")) + theme_void() + theme(aspect.ratio = 1, legend.position = "bottom") + ggtitle("SPLIT-purified VisiumHD data colored by purification status")
...
*The functionality below currently depends on RCTD output. It will be refactored to support generic deconvolution inputs in a future release.*
...
vhd <- AddMetaData(vhd, RCTD@results$results_df) # for the spot_class
